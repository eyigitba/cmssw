...
void PtAssignmentEngineDxy::preprocessing_dxy(const EMTFTrack& track, emtf::Feature& feature) const {
  ...
  dPhi_12 = (data.delta_ph[0] != invalid_dphi) ? data.delta_ph[0] : 0;
  dPhi_13 = (data.delta_ph[1] != invalid_dphi) ? data.delta_ph[1] : 0;
  dPhi_14 = (data.delta_ph[2] != invalid_dphi) ? data.delta_ph[2] : 0;
  dPhi_23 = (data.delta_ph[3] != invalid_dphi) ? data.delta_ph[3] : 0;
  dPhi_24 = (data.delta_ph[4] != invalid_dphi) ? data.delta_ph[4] : 0;
  dPhi_34 = (data.delta_ph[5] != invalid_dphi) ? data.delta_ph[5] : 0;

  // Calculate delta theta
  dTh_12 = (data.delta_th[0] != invalid_dtheta) ? data.delta_th[0] : 0;
  dTh_13 = (data.delta_th[1] != invalid_dtheta) ? data.delta_th[1] : 0;
  dTh_14 = (data.delta_th[2] != invalid_dtheta) ? data.delta_th[2] : 0;
  dTh_23 = (data.delta_th[3] != invalid_dtheta) ? data.delta_th[3] : 0;
  dTh_24 = (data.delta_th[4] != invalid_dtheta) ? data.delta_th[4] : 0;
  dTh_34 = (data.delta_th[5] != invalid_dtheta) ? data.delta_th[5] : 0;
  ...
  // std::cout << x_dphi[0]<< " " <<      x_dphi[1]<< " " <<      x_dphi[2]<< " " <<      x_dphi[3]<< " " <<      x_dphi[4]<< " " <<    x_dphi[5]<< " " <<
  //             x_dphi_sign[0]<< " " <<      x_dphi_sign[1]<< " " <<      x_dphi_sign[2]<< " " <<      x_dphi_sign[3]<< " " <<      x_dphi_sign[4]<< " " <<    x_dphi_sign[5]<< " " <<
  //             x_dtheta[0]<< " " <<    x_dtheta[1]<< " " <<    x_dtheta[2]<< " " <<    x_dtheta[3]<< " " <<    x_dtheta[4]<< " " <<  x_dtheta[5]<< " " <<
  //             x_dtheta_sign[0]<< " " <<    x_dtheta_sign[1]<< " " <<    x_dtheta_sign[2]<< " " <<    x_dtheta_sign[3]<< " " <<    x_dtheta_sign[4]<< " " <<  x_dtheta_sign[5]<< " " <<
  //             x_csc_pattern[0]<< " " <<    x_csc_pattern[1]<< " " <<    x_csc_pattern[2]<< " " <<    x_csc_pattern[3]<< " " << x_trk_theta[0] << std::endl;
  return;
}
...
void PtAssignmentEngineDxy::call_tensorflow_dxy(const emtf::Feature& feature, emtf::Prediction& prediction) const {
  ...
  tensorflow::Tensor input(tensorflow::DT_FLOAT, {1, 29});
  std::vector<tensorflow::Tensor> outputs;
  // emtf_assert(feature.size() == emtf::NUM_FEATURES);
  emtf_assert(feature.size() == 29);
  ...
  // std::cout << prediction.at(0) << " " << prediction.at(1) << std::endl;
